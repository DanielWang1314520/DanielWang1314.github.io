...
def layer_fun(inputs, input_dim, output_dim,activation=None):  #输入数据，输入神经元数量，输出神经元数量，激活函数
    W = tf.Variable(tf.truncated_normal([input_dim,output_dim],stddev = 0.1))  #随机数生成的数据如果比标准差两倍还要大就要被替换掉
    b = tf.Variable(tf.zeros([output_dim]))  #初始化为0    
    X_Wb = tf.matmul(inputs,W)+b  
    if activation is None:
        outputs = X_Wb
    else:
        outputs = activation(X_Wb)
    return outputs
...
def train_and_test(images_train, labels_train, images_test,
    ...
    #隐层层为2层
    h1 = layer_fun(inputs=x,
              input_dim=784,
              output_dim=256,
              activation=tf.nn.relu)
    
    h2 = layer_fun(inputs=h1,
              input_dim=256,
              output_dim=64,
              activation=tf.nn.relu)
    
    h3 = layer_fun(inputs=h2,
        input_dim=64,
        output_dim=56,
        activation=tf.nn.relu)
